...
const getGeminiKey = () => {
    ...
    console.error('Available environment variables:', Object.keys(process.env).filter(key => !key.includes('_KEY') && !key.includes('SECRET') && !key.includes('PASSWORD')));
    // Return empty string to indicate missing key
    return '';
};
// Mock data for when no API key is available
const mockIngredientAnalysis = [
    {
        ingredient: "Sugar",
        healthRating: "Bad",
...
const analyzeIngredients = async (text, productInfo) => {
...
async function POST(request) {
    ...
    try {
        ...
        if (!text) {
            return server_1.NextResponse.json({ error: 'No ingredients provided' }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ error: error instanceof Error ? error.message : 'An unknown error occurred' }, { status: 500 });
    }
}
