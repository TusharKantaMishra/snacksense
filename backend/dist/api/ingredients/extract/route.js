...
async function POST(request) {
    try {
        ...
        const { text } = body;
        if (!text) {
            return server_1.NextResponse.json({ error: 'No text provided' }, { status: 400 });
        }
        ...
        return server_1.NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to extract ingredients' }, { status: 500 });
    }
}
function extractIngredientsData(text) {
...
function parseCompositionFormat(text) {
    const ingredients = [];
...
function parseIngredientsListFormat(text) {
    const ingredients = [];
    ...
    for (let part of parts) {
        part = part.trim();
        if (!part || part.toLowerCase().includes('may contain'))
            continue;
...
function parseNutritionFactsFormat(text) {
