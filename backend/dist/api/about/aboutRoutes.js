...
router.post('/', async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Failed to add content', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
// PUT /api/about
router.put('/', async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Failed to update content', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
...
router.get('/seed', async (req, res) => {
    try {
        ...
        const results = await Promise.all(fallbackContent.map(item => aboutCollection.add(item)));
        return res.status(200).json({ message: 'Seeds added successfully!', count: results.length });
    }
    catch (error) {
        console.error('Error seeding data:', error);
        return res.status(500).json({ message: 'Error seeding data', error: error instanceof Error ? error.message : 'Unknown error' });
    }
});
