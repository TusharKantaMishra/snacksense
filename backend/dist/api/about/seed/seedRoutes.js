...
router.post('/', async (req, res) => {
    try {
        ...
        if (!snapshot.empty) {
            ...
            return res.status(200).json({ message: 'Seed skipped. Content already exists.', count: snapshot.size });
        }
        ...
        return res.status(201).json({ message: 'Seed completed successfully', count: seedData.length });
    }
    catch (error) {
        console.error('Seed error:', error);
        return res.status(500).json({ error: 'Failed to seed content', details: error instanceof Error ? error.message : 'Unknown error' });
    }
});
